...
<div>
    ...
    <asp:HiddenField runat="server" ClientIDMode="Static" ID="IDC" />
...
<div id="MessageOK" class="OKStatus" runat="server">
    Movimiento guardado...
</div>
...
<div id="DataCliente" runat="server">


</div>
<div id="DetalleCC" runat="server">
    <table style="width:100%">
        <tbody id="TBDetalleCC" runat="server">
            <tr class="metroheader">
                <th>Fecha</th>
                <th>Detalle</th>
...
<div id="DialogMov">
    <div>
      <span class="FormLabel">Monto:</span>
      <asp:TextBox ID="txtMonto" runat="server" ClientIDMode="Static" CssClass="AtroxTextBox"></asp:TextBox>
    </div>
    <div>
...
<script>
    ...
    function mv(mvtype)
    {
        var valmonto = $("#txtMonto").val();
        AddMov(mvtype, valmonto,IDCC);
    }

    function ShowMovOption(IdC)
    {
        $("#DialogMov").dialog('open');
        IDCC = IdC;
    ...
    function AddMov(mov,value,idc)
    {
        window.location.href = RawHost + "?mov=" + mov + "&value=" + value + "&idcm=" + idc;
    }
    ...
    function VCC(IDC)
    {
        window.location.href = RawHost + "?VCC=" + IDC;
    }
    ...
    function InterpeterJason(DATA)
    {
        ...
        for (i = 0; i < Clientes.length; i++)
        {
            ...
            var r = "<tr class='animationline resultline metro[pi]line'><td class='descripcionarticulo' style='text-decoration:[td]'>[RS]<span style='color:red'>[Susp]</span></td><td class='fixcell'>[L]</td><td class='fixcell'>[D]</td><td class='fixcell'>[E]</td><td><div class='buttoncell' onclick='editclient([IDC])'>Editar</div></td><td><div class='buttoncell' onclick='VCC([1IDCC])'>Cuenta Corriente</div></td><td><div class='buttoncell' onclick='ShowMovOption([2IDCC])'>Movimiento</div></td></tr>"
            r = r.replace("[pi]", c);
            ...
            r = r.replace("[1IDCC]", Clientes[i]["ID"]);
            r = r.replace("[2IDCC]", Clientes[i]["ID"]);
